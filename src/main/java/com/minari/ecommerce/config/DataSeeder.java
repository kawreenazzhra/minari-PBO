...
public class DataSeeder implements CommandLineRunner {
    ...
    private void createUsers() {
        ...
        try {
            ...
            String[][] customers = {
                {"diana@minari.com", "Diana Putri", "+6281122334455"},
                {"rina@minari.com", "Rina Santoso", "+6285544332211"},
                {"maya@minari.com", "Maya Wijaya", "+6289988776655"}
            };

            for (String[] customerData : customers) {
                if (userRepository.findByEmail(customerData[0]).isEmpty()) {
                    RegisteredCustomer customer = new RegisteredCustomer();
                    customer.setEmail(customerData[0]);
                    customer.setPassword(passwordEncoder.encode("password123"));
                    customer.setFullName(customerData[1]);
                    customer.setPhone(customerData[2]);
                    customer.setIsActive(true);

                    userRepository.save(customer);
                    userRepository.flush();
                    System.out.println("   ✅ Customer created: " + customerData[0] + " / password123");
                }
            }
        } catch (Exception e) {
