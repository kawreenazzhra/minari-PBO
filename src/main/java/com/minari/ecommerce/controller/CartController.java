...
public class CartController {
    ...
    public String viewCart(Model model, Principal principal, HttpSession session) {
        List<CartSessionItem> displayItems = new ArrayList<>();

        if (principal != null) {
            // Authenticated user - convert database cart to display items
            ...
            if (userCart != null && userCart.getItems() != null) {
                for (com.minari.ecommerce.entity.CartItem item : userCart.getItems()) {
                    Product p = item.getProduct();
                    displayItems.add(new CartSessionItem(
                            p.getId(),
                            p.getName(),
                            p.getPrice(),
                            "One Size",
            ...
            if (guestCart != null) {
                displayItems.addAll(guestCart);
            }
            model.addAttribute("isGuest", true);
        }

        model.addAttribute("cart", new CartSessionData(displayItems));
        return "cart/view";
    }
